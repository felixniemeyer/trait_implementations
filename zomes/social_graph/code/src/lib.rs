...
use hdk::prelude::{EntryType, LinkMatch};
...
pub fn handle_make_test_entry(message: String) -> ZomeApiResult<HashString> {
    ...
    hdk::link_entries(&agent_address, &entry_address, "makes", "")?;
    Ok(entry_address)
}

pub fn handle_get_test_entry_addresses() -> ZomeApiResult<Vec<HashString>> {
    let agent_address = hdk::AGENT_ADDRESS.clone().into();
    match hdk::get_links(&agent_address, LinkMatch::Exactly("makes"), LinkMatch::Any) {
...
pub fn handle_get_test_entries() -> ZomeApiResult<Vec<TestEntry>> {
    let agent_address = hdk::AGENT_ADDRESS.clone().into();
    hdk::utils::get_links_and_load_type(&agent_address, LinkMatch::Exactly("makes"), LinkMatch::Any)
}
...
pub fn handle_get_followings(agent_addr: HashString) -> ZomeApiResult<Vec<HashString>> {
    ...
    match hdk::get_links(&anchor_addr, LinkMatch::Exactly("follows"), LinkMatch::Any) {
        Ok(result) => Ok(result.addresses()),
        Err(err) => Err(err),
    }
}
...
pub fn handle_request_friendship(receiver_address: HashString) -> ZomeApiResult<()> {
    let sender_address = hdk::AGENT_ADDRESS.clone().into();
